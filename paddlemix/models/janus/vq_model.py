...
class Encoder(paddle.nn.Layer):
    ...
    def __init__(self, in_channels=3, ch=128, ch_mult=(1, 1, 2, 2, 4),
        num_res_blocks=2, norm_type='group', dropout=0.0, resamp_with_conv=
        True, z_channels=256):
        super().__init__()
        self.num_resolutions = len(ch_mult)
        self.num_res_blocks = num_res_blocks
        self.conv_in = paddle.nn.Conv2D(in_channels=in_channels,
            out_channels=ch, kernel_size=3, stride=1, padding=1)
        ...
        for i_level in range(self.num_resolutions):
            ...
            for _ in range(self.num_res_blocks):
                res_block.append(ResnetBlock(block_in, block_out, dropout=
                    dropout, norm_type=norm_type))
                block_in = block_out
        ...
        self.mid.append(ResnetBlock(block_in, block_in, dropout=dropout,
            norm_type=norm_type))
        ...
        self.conv_out = paddle.nn.Conv2D(in_channels=block_in, out_channels
            =z_channels, kernel_size=3, stride=1, padding=1)
...
class VectorQuantizer(paddle.nn.Layer):
    ...
    def __init__(self, n_e, e_dim, beta, entropy_loss_ratio, l2_norm,
        ...
        if self.l2_norm:
            self.embedding.weight.data = paddle.nn.functional.normalize(x=
                self.embedding.weight.data, p=2, axis=-1)
        if self.show_usage:
    ...
    def forward(self, z):
        ...
        if self.l2_norm:
            ...
            z_flattened = paddle.nn.functional.normalize(x=z_flattened, p=2,
                axis=-1)
            embedding = paddle.nn.functional.normalize(x=self.embedding.
                weight, p=2, axis=-1)
        else:
            embedding = self.embedding.weight
        d = paddle.sum(x=z_flattened ** 2, axis=1, keepdim=True) + paddle.sum(x
            =embedding ** 2, axis=1) - 2 * paddle.einsum('bd,dn->bn',
            z_flattened, paddle.einsum('n d -> d n', embedding))
        ...
        return z_q, (vq_loss, commit_loss, entropy_loss), (perplexity,
...
class Upsample(paddle.nn.Layer):
    ...
    def forward(self, x):
        if x.dtype != 'float32':
            x = paddle.nn.functional.interpolate(x=x.to('float32'),
                scale_factor=2.0, mode='nearest').to('bfloat16')
        else:
            x = paddle.nn.functional.interpolate(x=x, scale_factor=2.0,
                mode='nearest')
        if self.with_conv:
            x = self.conv(x)
        return x
...
def compute_entropy_loss(affinity, loss_type='softmax', temperature=0.01):
    ...
    log_probs = paddle.nn.functional.log_softmax(x=flat_affinity + 1e-05,
        axis=-1)
    ...
    sample_entropy = -paddle.mean(x=paddle.sum(x=target_probs * log_probs,
        axis=-1))
...
class VQModel(paddle.nn.Layer):
    ...
    def __init__(self, config: ModelArgs):
        super().__init__()
        self.config = config
        self.encoder = Encoder(ch_mult=config.encoder_ch_mult, z_channels=
            config.z_channels, dropout=config.dropout_p)
        self.decoder = Decoder(ch_mult=config.decoder_ch_mult, z_channels=
            config.z_channels, dropout=config.dropout_p)
        self.quantize = VectorQuantizer(config.codebook_size, config.
            codebook_embed_dim, config.commit_loss_beta, config.
            entropy_loss_ratio, config.codebook_l2_norm, config.
            codebook_show_usage)
        self.quant_conv = paddle.nn.Conv2D(in_channels=config.z_channels,
            out_channels=config.codebook_embed_dim, kernel_size=1)
        self.post_quant_conv = paddle.nn.Conv2D(in_channels=config.
            codebook_embed_dim, out_channels=config.z_channels, kernel_size=1)
