...
import numpy as np
from ppdiffusers.models.embeddings import Timesteps,TimestepEmbedding
...
class Downsample2D(paddle.nn.Layer):
    ...
    def __init__(self, channels: int, use_conv: bool=False, out_channels:
        ...
        stride=2, norm_type=None, eps=None, elementwise_affine=None, bias=True
        ):
        super().__init__()
...
class Upsample2D(paddle.nn.Layer):
    ...
    def __init__(self, channels: int, use_conv: bool=False,
        ...
        stride=2, norm_type=None, eps=None, elementwise_affine=None, bias=
        True, interpolate=True):
        super().__init__()
        self.channels = channels
        self.out_channels = out_channels or channels
        self.use_conv = use_conv
        ...
        if use_conv_transpose:
            ...
            conv = paddle.nn.Conv2DTranspose(in_channels=channels,
                ...
                stride=stride, padding=padding, bias_attr=bias)
        elif use_conv:
            if kernel_size is None:
                kernel_size = 3
            conv = paddle.nn.Conv2D(in_channels=self.channels, out_channels
...
class ConvNextBlock(paddle.nn.Layer):
    ...
    def __init__(self, channels, norm_eps, elementwise_affine, use_bias,
        hidden_dropout, hidden_size, res_ffn_factor: int=4):
        super().__init__()
        self.depthwise = paddle.nn.Conv2D(in_channels=channels,
            out_channels=channels, kernel_size=7, padding=3, groups=
            channels, bias_attr=use_bias)
        self.norm = LlamaRMSNorm(channels, norm_eps)
        self.channelwise_linear_1 = paddle.nn.Linear(in_features=channels,
            out_features=int(channels * res_ffn_factor), bias_attr=use_bias)
        self.channelwise_act = paddle.nn.GELU()
        self.channelwise_norm = GlobalResponseNorm(int(channels *
            res_ffn_factor))
        self.channelwise_linear_2 = paddle.nn.Linear(in_features=int(
            channels * res_ffn_factor), out_features=channels, bias_attr=
            use_bias)
        self.channelwise_dropout = paddle.nn.Dropout(p=hidden_dropout)
        self.cond_embeds_mapper = paddle.nn.Linear(in_features=hidden_size,
            out_features=channels * 2, bias_attr=use_bias)
    ...
    def forward(self, x, cond_embeds):
        ...
        x = x + x_res
        scale, shift = self.cond_embeds_mapper(paddle.nn.functional.silu(x=
            cond_embeds)).chunk(chunks=2, axis=1)
        x = paddle.add(shift[:, :, None, None], 1 * x * (1 + scale)[:, :,
            None, None])
        return x
...
class Patchify(paddle.nn.Layer):
    ...
    def __init__(self, in_channels, block_out_channels, patch_size, bias,
        elementwise_affine, eps, kernel_size=None):
        super().__init__()
        if kernel_size is None:
            kernel_size = patch_size
        self.patch_conv = paddle.nn.Conv2D(in_channels=in_channels,
            out_channels=block_out_channels, kernel_size=kernel_size,
            stride=patch_size, bias_attr=bias)
...
class Unpatchify(paddle.nn.Layer):
    ...
    def __init__(self, in_channels, out_channels, patch_size, bias,
        elementwise_affine, eps):
        super().__init__()
        self.norm = LlamaRMSNorm(in_channels, eps)
        self.unpatch_conv = paddle.nn.Conv2D(in_channels=in_channels,
...
class UVitBlock(paddle.nn.Layer):
    ...
    def __init__(self, channels, out_channels, num_res_blocks, stride,
        hidden_size, hidden_dropout, elementwise_affine, norm_eps, use_bias,
        downsample: bool, upsample: bool, res_ffn_factor: int=4, seq_len=
        None, concat_input=False, original_input_channels=None, use_zero=
        True, norm_type='RMS'):
        super().__init__()
        self.res_blocks = paddle.nn.LayerList()
        for i in range(num_res_blocks):
            conv_block = ConvNextBlock(channels, norm_eps,
                elementwise_affine, use_bias, hidden_dropout, hidden_size,
                res_ffn_factor=res_ffn_factor)
            self.res_blocks.append(conv_block)
        if downsample:
            self.downsample = Downsample2D(channels=channels, out_channels=
                out_channels, use_conv=True, name='Conv2d_0', kernel_size=3,
                padding=1, stride=stride, norm_type='rms_norm', eps=
                norm_eps, elementwise_affine=elementwise_affine, bias=use_bias)
        else:
            self.downsample = None
        if upsample:
            self.upsample = Upsample2D(channels=channels, out_channels=
                out_channels, use_conv_transpose=False, use_conv=True,
                kernel_size=3, padding=1, stride=stride, name='conv',
                norm_type='rms_norm', eps=norm_eps, elementwise_affine=
                elementwise_affine, bias=use_bias, interpolate=True)
        else:
            self.upsample = None
...
class ShallowUViTEncoder(paddle.nn.Layer):
    ...
    def __init__(self, input_channels=3, stride=4, kernel_size=7, padding=
        None, block_out_channels=(768,), layers_in_middle=2, hidden_size=
        2048, elementwise_affine=True, use_bias=True, norm_eps=1e-06,
        dropout=0.0, use_mid_block=True, **kwargs):
        super().__init__()
        self.time_proj = Timesteps(
            block_out_channels[0], flip_sin_to_cos=True, downscale_freq_shift=0
        )
        self.time_embed = TimestepEmbedding(
            block_out_channels[0], hidden_size)
        if padding is None:
            padding = math.ceil(kernel_size - stride)
        self.in_conv = paddle.nn.Conv2D(in_channels=input_channels,
            out_channels=block_out_channels[0], kernel_size=kernel_size,
            stride=stride, padding=padding)
        if use_mid_block:
            self.mid_block = UVitBlock(block_out_channels[-1],
                block_out_channels[-1], num_res_blocks=layers_in_middle,
                hidden_size=hidden_size, hidden_dropout=dropout,
                elementwise_affine=elementwise_affine, norm_eps=norm_eps,
                use_bias=use_bias, downsample=False, upsample=False, stride
                =1, res_ffn_factor=4)
        else:
            self.mid_block = None
...
class ShallowUViTDecoder(paddle.nn.Layer):
    ...
    def __init__(self, in_channels=768, out_channels=3, block_out_channels:
        Tuple[int]=(768,), upsamples=2, layers_in_middle=2, hidden_size=
        2048, elementwise_affine=True, norm_eps=1e-06, use_bias=True,
        dropout=0.0, use_mid_block=True, **kwargs):
        super().__init__()
        if use_mid_block:
            self.mid_block = UVitBlock(in_channels + block_out_channels[-1],
                block_out_channels[-1], num_res_blocks=layers_in_middle,
                hidden_size=hidden_size, hidden_dropout=dropout,
                elementwise_affine=elementwise_affine, norm_eps=norm_eps,
                use_bias=use_bias, downsample=False, upsample=False, stride
        ...
        for rank in range(upsamples):
            ...
            self.out_convs.append(Unpatchify(curr_in_channels,
                curr_out_channels, patch_size=2, bias=use_bias,
                elementwise_affine=elementwise_affine, eps=norm_eps))
