...
def calculate_scores(data):
    ...
    for entry in data:
        ...
        if question_type == 'count':
            ...
            if is_correct_area(response, answer):
                type_correct[question_type] += 1
        elif response and response.lower() == answer.lower():
            type_correct[question_type] += 1
    type_scores = {}
    ...
    total_score = round(total_correct / total_count, 4
        ) if total_count > 0 else 0.0
    total_correct_useful = sum([v for k, v in type_correct.items() if k not in
        ['count', 'area']])
    total_count_useful = sum([v for k, v in type_counts.items() if k not in
        ['count', 'area']])
    total_score_useful = round(total_correct_useful / total_count_useful, 4
        ) if total_count_useful > 0 else 0.0
    print(f'type_scores={type_scores!r}')
    print(f'total_score_useful={total_score_useful!r}')
...
if __name__ == '__main__':
    ...
    if 'outputs' in data:
        data = data['outputs']
    type_scores, total_score, total_score_useful, type_counts = (
        calculate_scores(data))
    results = {'type_scores': type_scores, 'type_counts': type_counts,
        'total_score': total_score, 'total_score_useful':
        total_score_useful, 'outputs': data}
