...
from eval_utils import calculate_ins_level_acc, evaluate, parse_multi_choice_response, parse_open_response
if __name__ == '__main__':
    parser = ArgumentParser()
    parser.add_argument('--output_path', type=str, default=
        './example_outputs/qwen_vl/total_val_output.json', help=
        'The path to model output file.')
    parser.add_argument('--answer_path', type=str, default=
        './answer_dict_val.json', help='Answer file path.')
    ...
    for category in CAT_SHORT2LONG.values():
        ...
        for data_id, parsed_pred in cat_outputs.items():
            ...
            exampels_to_eval.append({'id': data_id, 'question_type':
                question_type, 'answer': cat_answers[data_id][
                'ground_truth'], 'parsed_pred': parsed_pred})
    ...
    for domain, in_domain_cats in DOMAIN_CAT2SUB_CAT.items():
        ...
        in_domain_data_num = sum([cat_results['num_example'] for
            cat_results in in_domain_cat_results.values()])
        printable_results['Overall-' + domain] = {'num': int(
            in_domain_data_num), 'acc': round(in_domain_ins_acc, 3)}
        for cat_name, cat_results in in_domain_cat_results.items():
            printable_results[cat_name] = {'num': int(cat_results[
                'num_example']), 'acc': round(cat_results['acc'], 3)}
    all_ins_acc = calculate_ins_level_acc(evaluation_result)
    printable_results['Overall'] = {'num': sum([cat_results['num_example'] for
        cat_results in evaluation_result.values()]), 'acc': round(
        all_ins_acc, 3)}
