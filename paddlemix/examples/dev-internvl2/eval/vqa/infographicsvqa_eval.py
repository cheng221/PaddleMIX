...
answer_types = {'image span': 'Image-Span', 'question span':
    'Question-Span', 'multiple spans': 'Multi-Span', 'non span': 'None span'}
evidence_types = {'table/list': 'Table/list', 'textual': 'Text',
    'photo/pciture/visual_objects': 'Visual/Layout', 'figure': 'Figure',
    'map': 'Map'}
reasoning_requirements = {'comparison': 'Sorting', 'arithmetic':
    'Arithmetic', 'counting': 'Counting'}
...
def validate_data(gtFilePath, submFilePath):
    ...
    res_id_to_index = {int(r['questionId']): ix for ix, r in enumerate(
        submJson)}
    ...
    for gtObject in gtJson['data']:
        try:
            ...
            raise Exception('The Det file is not valid. Question ' + str(
                gtObject['questionId']) + ' not present')
        else:
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    if show_scores_per_answer_type:
        ...
        reasoningTypeTotalScore = {x: (0) for x in reasoning_requirements.
            keys()}
        reasoningTypeNumQuestions = {x: (0) for x in reasoning_requirements
            .keys()}
    for gtObject in gtJson['data']:
        q_id = int(gtObject['questionId'])
        res_ix = res_id_to_index[q_id]
        ...
        if q_id in question_ids_to_exclude:
            ...
            for answer in gtObject['answers']:
                ...
                det_answer = ' '.join(detObject['answer'].strip().lower().
                    split())
                dist = levenshtein_distance(gt_answer, det_answer)
                length = max(len(answer.upper()), len(detObject['answer'].
                    upper()))
                values.append(0.0 if length == 0 else float(dist) / float(
                    length))
        ...
        perSampleMetrics[str(gtObject['questionId'])] = {'score':
            question_result, 'question': gtObject['question'], 'gt':
            gtObject['answers'], 'det': detObject['answer'], 'info': info}
        row = row + 1
    methodMetrics = {'score': 0 if len(gtJson['data']) == 0 else totalScore /
    ...
    if show_scores_per_answer_type:
        for a_type, ref in answer_types.items():
            answer_types_scores[ref] = 0 if len(gtJson['data']
                ) == 0 else answerTypeTotalScore[a_type
                ] / answerTypeNumQuestions[a_type]
        for e_type, ref in evidence_types.items():
            evidence_types_scores[ref] = 0 if len(gtJson['data']
                ) == 0 else evidenceTypeTotalScore[e_type
                ] / evidenceTypeNumQuestions[e_type]
        for r_type, ref in reasoning_requirements.items():
            operation_types_scores[ref] = 0 if len(gtJson['data']
                ) == 0 else reasoningTypeTotalScore[r_type
                ] / reasoningTypeNumQuestions[r_type]
    resDict = {'result': methodMetrics, 'scores_by_types': {'answer_types':
        answer_types_scores, 'evidence_types': evidence_types_scores,
        'operation_types': operation_types_scores}, 'per_sample_result':
        perSampleMetrics}
    return resDict
...
def display_results(results, show_answer_types):
    ...
    if show_answer_types:
        ...
        for a_type in answer_types.values():
            print('\t{:12s} {:2.4f}'.format(a_type, results[
                'scores_by_types']['answer_types'][a_type]))
        print('\nEvidence types:')
        for e_type in evidence_types.values():
            print('\t{:12s} {:2.4f}'.format(e_type, results[
                'scores_by_types']['evidence_types'][e_type]))
        print('\nOperation required:')
        for r_type in reasoning_requirements.values():
            print('\t{:12s} {:2.4f}'.format(r_type, results[
                'scores_by_types']['operation_types'][r_type]))


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description=
        'InfographVQA evaluation script.')
    parser.add_argument('-g', '--ground_truth', type=str, help=
        'Path of the Ground Truth file.', required=True)
    parser.add_argument('-s', '--submission_file', type=str, help=
        "Path of your method's results file.", required=True)
