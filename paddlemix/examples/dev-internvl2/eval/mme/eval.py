...
# from paddlemix.models.internvl2.internlm2 import Qwen2Tokenizer
...
if __name__ == '__main__':
    ...
    kwargs = {} if args.auto else {}
    tokenizer = Qwen2Tokenizer.from_pretrained(args.checkpoint, use_fast=False,trust_remote_code=True)
    model = InternVLChatModel.from_pretrained(args.checkpoint, dtype=args.dtype).eval()
    ...
    for filename in os.listdir(args.root):
        ...
        for line in tqdm(lines):
            ...
            img_path = os.path.join(
                '../../data/mme/MME_Benchmark_release_version', filename, img)
            assert os.path.exists(img_path), img_path
            pixel_values = load_image(img_path, image_size).cuda(blocking=True
                ).to('bfloat16')
