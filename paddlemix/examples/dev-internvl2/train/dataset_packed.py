...
# paddlenlp.trainer paddlenlp 没有labelsmoother
...
class PackedDataset(paddle.io.IterableDataset):
    ...
    def __init__(self, tokenizer, data_rank, data_world_size, datasets:
        List, dataset_weight: List[int]=None, num_images_expected: int=6,
        max_packed_tokens: int=32768, max_buffer_size: int=100, log_freq:
        int=1000000, strict_mode: bool=False, debug_mode: bool=False,
        replacement: bool=True, allow_overflow: bool=True, allow_empty_data:
        bool=False, allow_deduplicated_ds_name: bool=False):
        super().__init__()
        ...
        self.img_token_id = self.tokenizer.convert_tokens_to_ids(
            IMG_CONTEXT_TOKEN)
        self.img_end_token_id = self.tokenizer.convert_tokens_to_ids(
            IMG_END_TOKEN)
        ...
        self.dataset_weight_orig = [(w / sum(dataset_weight)) for w in
            dataset_weight]
        ...
        self._state_dict = {'sample_info': {d.ds_name: (0) for d in self.
            datasets}}
    ...
    def _should_log(self):
        worker_id = 0 if paddle.io.get_worker_info(
            ) is None else paddle.io.get_worker_info().id
        num_workers = 1 if paddle.io.get_worker_info(
            ) is None else paddle.io.get_worker_info().num_workers
    ...
    def next_data(self, current_dataset_idx):
        while True:
            try:
                current_sample = next(self.dataset_iter_list[
                    current_dataset_idx])
                break
            except StopIteration:
                if self.replacement:
                    try:
                        self.dataset_iter_list[current_dataset_idx] = iter(self
                            .datasets[current_dataset_idx])
                        current_sample = next(self.dataset_iter_list[
                            current_dataset_idx])
                        break
                    except:
                        ...
                        if len(self.datasets) == 0:
                            raise StopIteration
                        current_dataset_idx = np.random.choice(len(self.
                            datasets))
                else:
                    self.datasets.pop(current_dataset_idx)
                    self.dataset_iter_list.pop(current_dataset_idx)
        ...
        current_sample['type_ids'] = paddle.zeros_like(x=current_sample[
            'input_ids']) + current_dataset_idx
        ...
        self._state_dict[current_ds_name][self.worker_state_key].update(**
            meta_info)
        self._state_dict['sample_info'][self.datasets[current_dataset_idx].
            ds_name] += 1
    ...
    def find_buffer(self, buffer_list, new_sample):
        ...
        for buffer_idx, buffer in enumerate(buffer_list):
            ...
            if (num_images_buffer + num_images_current <= self.
                num_images_expected):
                num_merged_tokens = new_sample['input_ids'].shape[0] + buffer[
                    'input_ids'].shape[0]
                if num_merged_tokens <= self.max_packed_tokens:
                    find = True
                    find_idx = buffer_idx
                    break
                if self.allow_overflow and len(buffer_list
    ...
    def update_buffer(self, buffer, new_sample):
        if buffer is None:
            new_sample['data_index'] = paddle.zeros_like(x=new_sample[
                'input_ids'])
            return new_sample
        new_sample['data_index'] = paddle.ones_like(x=new_sample['input_ids']
            ) + buffer['data_index'][-1].item()
    ...
    @staticmethod
    def split_buffer(buffer, max_tokens, img_start_token_id, img_token_id,
        img_end_token_id):
        ...
        def _split(sample_to_split, left_idx, right_idx, left_img_idx,
            right_img_idx):
            ...
            for k in sample_to_split:
                if k in ['input_ids', 'labels', 'attention_mask',
                    'position_ids', 'data_index', 'type_ids']:
        ...
        while buffer['input_ids'].shape[0] > max_tokens:
            img_start_idx_list = (buffer['input_ids'] == img_start_token_id
                ).nonzero().squeeze(axis=1).tolist()
            img_end_idx_list = (buffer['input_ids'] == img_end_token_id
                ).nonzero().squeeze(axis=1).tolist()
            ...
            if _image_is_splitted(buffer['input_ids'], max_tokens):
                ...
                cut_left_img_idx = (cut_right_img_idx if cut_right_img_idx
                     is not None else buffer['pixel_values'].shape[0])
            left, right = _split(sample_to_split=buffer, left_idx=
                cut_left_idx, left_img_idx=cut_left_img_idx, right_idx=
                cut_right_idx, right_img_idx=cut_right_img_idx)
            assert (left['input_ids'] == img_end_token_id).sum() == (left[
                'input_ids'] == img_start_token_id).sum() == left[
                'pixel_values'].shape[0]
            if right is not None:
                assert (right['input_ids'] == img_end_token_id).sum() == (right
                    ['input_ids'] == img_start_token_id).sum() == right[
                    'pixel_values'].shape[0]
            if left['pixel_values'].shape[0
                ] >= 1 and PackedDataset.check_valid(left):
                splitted_buffer.append(left)
            if right is None or right['pixel_values'].shape[0] == 0:
                break
            buffer = right
            if buffer['input_ids'].shape[0
                ] <= max_tokens and PackedDataset.check_valid(buffer):
                splitted_buffer.append(buffer)
                break
        logger.debug(
            f'split a sample into {len(splitted_buffer)} samples, current max_tokens={max_tokens}'
            )
        return splitted_buffer

    def update_buffer_list(self, buffer_list, buffer_max_len_list, buffer):
        splitted_buffer = PackedDataset.split_buffer(buffer=buffer,
            max_tokens=self.max_packed_tokens, img_start_token_id=self.
            img_start_token_id, img_token_id=self.img_token_id,
        ...
        for each_buffer in splitted_buffer:
            ...
            if each_buffer['input_ids'].shape[0] >= self.max_packed_tokens:
                assert each_buffer['input_ids'].shape[0
            ...
            for buffer_idx in range(len(buffer_list)):
                if buffer_list[buffer_idx]['pixel_values'].shape[0
                    ] < num_images_new_sample:
                    find_idx = buffer_idx
                    break
            buffer_list.insert(find_idx, each_buffer)
        for i in range(1, len(buffer_list)):
            assert buffer_list[i - 1]['pixel_values'].shape[0] >= buffer_list[i
                ]['pixel_values'].shape[0]
    ...
    def pad_buffer(self, buffer):
        if buffer['pixel_values'].shape[0] == self.num_images_expected:
            return buffer
        num_pad_images = self.num_images_expected - buffer['pixel_values'
            ].shape[0]
        pad_images = paddle.stack(x=[paddle.zeros_like(x=buffer[
            'pixel_values'][0]) for _ in range(num_pad_images)])
        pad_image_flags = paddle.to_tensor(data=[0] * num_pad_images, dtype
            ='int64')
        buffer['pixel_values'] = paddle.concat(x=[buffer['pixel_values'],
            pad_images])
        buffer['image_flags'] = paddle.concat(x=[buffer['image_flags'],
            pad_image_flags])
        return buffer
    ...
    def postprocess_buffer(self, buffer, custom_infos=None):
        ...
        if custom_infos is not None:
            buffer['custom_infos'] = {self.worker_state_key: copy.deepcopy(
                custom_infos)}
        return buffer
    ...
    def __iter__(self):
        ...
        if (self.worker_custom_infos is not None and self.worker_state_key in
            self.worker_custom_infos):
            custom_infos = self.worker_custom_infos[self.worker_state_key]
            if 'buffer_list' in custom_infos and isinstance(custom_infos[
                'buffer_list'], list):
            ...
            self.worker_custom_infos = None
        logger.debug(
            f'{self.__class__.__name__} Rank {self.data_rank} Worker {worker_id} begin to load data'
            )
        while True:
            self.dataset_weight = [(w / sum(self.dataset_weight)) for w in
                self.dataset_weight]
            current_dataset_idx = rng.choice(len(self.dataset_iter_list), p
                =self.dataset_weight)
            try:
                ...
                while len(buffer_list) > 0:
                    if self.strict_mode:
                        yield self.postprocess_buffer(self.pad_buffer(
                            buffer_list.pop(0)))
                    else:
                        yield self.postprocess_buffer(buffer_list.pop(0))
                logger.info(
                    f'buffer_list is empty! (len(buffer_list)={len(buffer_list)!r})'
                    )
                return
            ...
            while len(buffer_max_len_list) > 0:
                if buffer_max_len_list[0]['pixel_values'].shape[0
                    ] != self.max_packed_tokens:
                    logger.debug(
                ...
                if self.strict_mode and buffer_max_len_list[0]['pixel_values'
                    ].shape[0] != self.num_images_expected:
                    yield self.postprocess_buffer(self.pad_buffer(
                        buffer_max_len_list.pop(0)), {'buffer_list':
                        buffer_list})
                else:
                    yield self.postprocess_buffer(buffer_max_len_list.pop(0
                        ), {'buffer_list': buffer_list})
            while len(buffer_list) > 0 and buffer_list[0]['pixel_values'
                ].shape[0] > self.num_images_expected:
                logger.error(
    ...
    @staticmethod
    def get_cu_seqlens_and_indexes(data_index: paddle.Tensor, input_ids:
        paddle.Tensor, labels: paddle.Tensor, len2weight: callable):
        indexes = []
        cu_seqlens = [0]
        loss_weight = []
        ...
        for i in range(start, end):
            ...
            assert num_tokens > 0
            curr_data_index = data_index[cu_seqlens[-2]:cu_seqlens[-2] +
                num_tokens]
        ...
        assert len(indexes) == data_index.shape[0
...
def packed_collate_fn(features, data_collator, len2weight: callable,
    max_item_length: int, micro_num: int=1, loss_reduction_all_gather: bool
    =False, pad_id: int=0):
    if not isinstance(features, list):
        features = [features]
    if len(features) > micro_num:
        raise NotImplementedError(
            f'len(features)={len(features)!r} > micro_num={micro_num!r}')
    ...
    while len(features) < micro_num:
        ...
        features[-1]['labels'] = paddle.full_like(x=features[-1]['labels'],
            fill_value=IGNORE_TOKEN_ID)
    indexes = []
    cu_seqlens = []
    ...
    for feat_idx, feat in enumerate(features):
        ...
        curr_cu_seqlens, curr_indexes, curr_loss_weight = (PackedDataset.
            get_cu_seqlens_and_indexes(data_index=data_index, input_ids=
            feat['input_ids'], labels=feat['labels'], len2weight=len2weight))
        ...
        if curr_cu_seqlens[-1] < max_item_length:
            ...
            curr_indexes.extend(list(range(max_item_length -
                curr_cu_seqlens[-2])))
        ...
        cu_num_images_list.append(cu_num_images_list[-1] + feat[
            'pixel_values'].shape[0])
    batch = data_collator(features=features, max_item_length=
        max_item_length, pad_id=pad_id)
    batch['loss_weight'] = paddle.where(condition=batch['labels'] ==
        IGNORE_TOKEN_ID, x=0, y=batch['loss_weight']).tolist()
    batch['attention_mask'] = paddle.stack(x=cu_seqlens)
    batch['loss_reduction_all_gather'] = loss_reduction_all_gather
    batch['statistics'] = paddle.to_tensor(data=[num_samples,
        num_padding_tokens, batch['image_flags'].size - batch['image_flags'
        ].sum().item()], dtype='int64')
    batch.pop('type_ids')
    return batch
