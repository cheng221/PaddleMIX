...
from paddlenlp.transformers.gpt import GPTTokenizer
...
sys.path.append(os.path.join(os.path.dirname(__file__), os.pardir))  # for resolve import
...
def main():
    ...
    tokenizer = GPTTokenizer.from_pretrained(tokenizer_path, add_eos_token=False)
    ...
    if model_args.model_name_or_path is not None:
        ...
        model = InternVLChatModel.from_pretrained(model_args.model_name_or_path, dtype="bfloat16", config=config)
    else:
        logger.info("Loading ViT-6B...")
        vision_config = InternVisionConfig.from_pretrained(model_args.vision_path)
        vision_config.drop_path_rate = model_args.drop_path_rate
        vision_model = InternVisionModel.from_pretrained(
            model_args.vision_path, dtype="bfloat16", config=vision_config
        )
        ...
        llm = model_type.from_pretrained(
            model_args.llm_path, dtype="bfloat16", config=llm_config, trust_remote_code=True
        )
