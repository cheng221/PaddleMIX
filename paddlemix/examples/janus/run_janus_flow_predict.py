...
sft_format = vl_chat_processor.apply_sft_template_for_multi_turn_prompts(
    conversations=conversation, sft_format=vl_chat_processor.sft_format,
    system_prompt='')
prompt = sft_format + vl_chat_processor.image_start_tag

@paddle.no_grad()
def generate(vl_gpt,vl_chat_processor,tokenizer,prompt, cfg_weight: float=2.0, num_inference_steps: int=30,batch_size: int=1):
    ...
    attention_mask = paddle.ones(shape=(2*batch_size, tuple(inputs_embeds.shape)[1] +
        577))
    ...
    for step in tqdm(range(num_inference_steps)):
        ...
        llm_emb = paddle.concat(x=[inputs_embeds, t_emb.unsqueeze(axis=1),
            z_emb], axis=1) if step==0 else paddle.concat(x= [t_emb.unsqueeze(axis=1),z_emb], axis=1)
        ...
        outputs = vl_gpt.language_model.llama(position_ids=position_ids,
            inputs_embeds=llm_emb,use_cache=True, attention_mask=attention_mask,
            past_key_values=past_key_values if step!=0 else None,return_dict=True)
        if step ==0:
            past_key_values = []
        ...
        hidden_states = vl_gpt.vision_gen_dec_aligner(vl_gpt.
            vision_gen_dec_aligner_norm(hidden_states[:, -576:, :]))
        hidden_states = hidden_states.reshape([tuple(z_emb.shape)[0], 24, 24,
            768]).transpose(perm=[0, 3, 1, 2])
        v = vl_gpt.vision_gen_dec_model(hidden_states, hs, t_emb)
        v_cond, v_uncond = paddle.chunk(x=v, chunks=2)
        v = cfg_weight * v_cond - (cfg_weight - 1.0) * v_uncond
        z = z + dt * v
    decoded_image = vae.decode(z / vae.config.scaling_factor).sample
    images = decoded_image.astype(dtype='float32').clip_(min=-1.0, max=1.0
        ).transpose(perm=[0, 2, 3, 1]).cpu().numpy()
    images = ((images + 1) / 2.0 * 255).astype(np.uint8)
        
    os.makedirs('janus_flow_generated_samples', exist_ok=True)
    for i in range(batch_size):
        save_path = os.path.join('janus_flow_generated_samples', 'img_{}.jpg'.format(i))
