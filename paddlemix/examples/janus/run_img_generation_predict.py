...
@paddle.no_grad()
def generate(mmgpt, vl_chat_processor, prompt: str,
            temperature: float=1, parallel_size: int=2,
            cfg_weight: float=5, image_token_num_per_image: int=576,
            img_size:int=384, patch_size: int=16):
    input_ids = vl_chat_processor.tokenizer.encode(prompt)
    input_ids = paddle.to_tensor(data=input_ids.input_ids, dtype='int64')
    tokens = paddle.zeros(shape=(parallel_size * 2, len(input_ids)), dtype=
        'int32')
    ...
    generated_tokens = paddle.zeros(shape=(parallel_size,
        image_token_num_per_image), dtype='int32')
    ...
    for i in tqdm(range(image_token_num_per_image)):
        ...
        next_token = paddle.concat(x=[next_token.unsqueeze(axis=1),
            next_token.unsqueeze(axis=1)], axis=1).reshape([-1])
        img_embeds = mmgpt.prepare_gen_img_embeds(next_token)
        inputs_embeds = img_embeds.unsqueeze(axis=1)
        
    dec = mmgpt.gen_vision_model.decode_code(generated_tokens.to(dtype=
        'int32'), shape=[parallel_size, 8, img_size // patch_size, img_size //
        patch_size])
    dec = dec.to('float32').cpu().numpy().transpose(0, 2, 3, 1)
    dec = np.clip((dec + 1) / 2 * 255, 0, 255)
    visual_img = np.zeros((parallel_size, img_size, img_size, 3), dtype=np.
        uint8)
    visual_img[:, :, :] = dec
